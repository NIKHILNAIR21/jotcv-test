...
import { createCertificate } from "../services/ApiServices";
...
import {
  ...
  restoreCertificate,
  updateCertificate,
} from "../slice/CertificateInfoSlice";
...
const CertificateInfo = () => {
  ...
  const initialValues = useSelector((state) => state?.certificate?.formData);
  const addCertificateHandler = (values) => {
    dispatch(addCertificate({ name: values?.name }));
    dispatch(restoreCertificate());
  };
  const removeCertificateHandler = (index) => {
  ...
  const handleSubmit = async () => {
    if (Certificates?.length !== 0) {
      ...
      formData.append("total_certificates", Certificates?.length);
      for (let index = 0; index < Certificates?.length; index++) {
        formData.append(`certificate${index + 1}`, Certificates[index]?.name);
      }
      try {
            ...
            <div
              ...
              <Field
                ...
                onChange={(e) => {
                  ...
                  dispatch(updateCertificate({ name: e.target.value }));
                }}
                      ...
                      >
                        ...
                        >
                          ...
                          <button
                            ...
                            onClick={() => removeCertificateHandler(index)}
                          >
                            X
                          </button>
                        </div>
