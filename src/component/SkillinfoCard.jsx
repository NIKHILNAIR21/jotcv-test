      ...
      >
        <AccordionHeader onClick={() => handleOpen(1)}>
          <p className="flex text-lg text-black font-bold items-center gap-2">
            <img className="w-5" src={skill} /> Skill{" "}
          </p>
        </AccordionHeader>
        <AccordionBody>
          <div className="flex flex-wrap">
            {resumeData?.skills?.length != 0 &&
              resumeData?.skills?.map((items, index) => (
                <div
                  key={items?.id}
                  className="flex justify-between items-center gap-2 hover:text-white m-2 w-fit bg-blue-100 p-3 rounded-full transition-all delay-100 hover:bg-blue-600"
                >
          ...
          <div className="flex justify-center">
            <button
              className="p-2 px-3 rounded-full font-medium text-blue-600  border hover:bg-blue-600 hover:text-white transition-all delay-100 "
              onClick={() => dispatch(setActiveForm("SkillInfoFormShow"))}
            >
              <span className="font-bold ">+</span> Skills
            </button>
          </div>
        </AccordionBody>
      </Accordion>
